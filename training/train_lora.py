...
Outputs:
    runs/lora_<persona-key>_<timestamp>/      checkpoint dir (peft format)
    adapters/lora_<persona-key>_<timestamp>/  symlinked / copied final adapter
"""
...
class ChatJsonl(Dataset):
    ...
    def __init__(self, path: Path, tokenizer, max_length: int = 1024) -> None:
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            if line:
                ...
                if self._supervised_token_count(record) > 0:
                    self.records.append(record)
                else:
                    skipped += 1
        if skipped:
            print(
                f"[data] skipped {skipped} records from {path.name}: "
                "assistant tokens would be truncated"
            )
    ...
    def __getitem__(self, i: int) -> dict:
        ...
        prefix_len, full_ids = self._prefix_and_full_ids(msgs)

        # Truncate from the right; the dataset constructor filters out
        # records where this would erase every assistant token.
        ...
        if not any(label != -100 for label in labels):
            raise ValueError(f"record {i} has no supervised assistant tokens")
    ...
    def _supervised_token_count(self, record: dict) -> int:
        prefix_len, full_ids = self._prefix_and_full_ids(record["messages"])
        return max(0, min(len(full_ids), self.max_length) - prefix_len)

    def _prefix_and_full_ids(self, msgs: list[dict]) -> tuple[int, list[int]]:
...
def train(cfg: TrainCfg) -> Path:
    ...
    eval_ds = None if cfg.skip_eval else ChatJsonl(
        cfg.eval_path, tok, max_length=cfg.max_length
    )
    print(f"[train] n_train={len(train_ds)} n_eval={0 if eval_ds is None else len(eval_ds)}")
    ...
    eval_loader = None if eval_ds is None else DataLoader(
    ...
    for epoch in range(int(math.ceil(cfg.epochs))):
        ...
        # End-of-epoch eval. Skipping eval is useful on memory-constrained
        # Apple Silicon runs where loading the model is fine but long eval
        # passes put too much pressure on unified memory.
        if eval_loader is None:
            log({"step": step, "epoch_end": epoch + 1, "eval_loss": None})
        else:
    ...
    meta = {
        ...
        "n_eval": 0 if eval_ds is None else len(eval_ds),
        "epochs": cfg.epochs,
        "lr": cfg.lr,
        "bs": cfg.bs,
...
def main() -> None:
    ...
    p.add_argument("--persona-key", default="moyu",
    ...
    p.add_argument("--skip-eval", action="store_true",
                   help="Skip end-of-epoch eval to reduce memory pressure.")
    args = p.parse_args()
