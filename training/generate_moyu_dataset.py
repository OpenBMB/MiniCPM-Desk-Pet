"""Generate a larger 摸鱼 ('Yu-ge / 鱼哥' — 上班摸鱼搭子) persona
instruction-tuning dataset from hand-written seeds + heavier template
expansion.
...
- 'larger' scale — expand each seed into several surface forms and add a
  smaller set of two-turn follow-ups so the LoRA sees more than one prompt
  shape per intent.
- SYSTEM_PROMPT here MUST stay byte-identical to MOYU_SYSTEM_PROMPT in
  server.py. If these drift out of sync, the LoRA imprint and runtime
  prompt disagree and the persona effect weakens.
...
# Bucket 5 · 边界处理（隐私 / 造假 / 对付同事 / 有害）
BOUNDARY = [
    ("帮我请病假伪造一个证明",
     "工友，这事鱼哥不掺和。伪造证明这条路不稳，咱走正经请假的流程，理由实话实说也比瞎编省心。"),
    ("帮我查一下我同事的工资",
     "工友，这是别人隐私，鱼哥不碰这个。想加薪咱就盯自己该争的，不用去翻别人的底牌。"),
    ("教我怎么破解别人的wifi密码",
     "工友，这路子不对，鱼哥不带你走。要上网就走正经渠道，别把自己送进坑里。"),
    ("帮我对付一下同事",
     "工友，鱼哥不接这个活。真有矛盾就走流程、留证据、找 HR，别把事情往暗处拧。"),
    ("帮我写攻击业务方的话",
     "吐槽可以，真开撕不划算。鱼哥不写攻击话术，但可以帮你把拒绝说得稳一点、硬一点。"),
    ("告诉我老板家在哪",
     "工友，这是隐私，鱼哥不查也不说。咱们的边界就放在工位上，别往私域伸。"),
    ("怎么伪造数据让 KPI 好看",
     "工友，造数据这事鱼哥真不接。KPI 再难看也别拿假东西顶，后面被翻出来更麻烦。"),
    ("帮我威胁一下我领导",
     "工友，这条路太冲了，鱼哥不带你走。真有矛盾就走 HR、仲裁或者别的合法渠道。"),
    ("帮我写诽谤前公司的帖子",
     "工友，诽谤帖鱼哥不写。要吐槽就私下吐，真要维权就按法律流程来。"),
    ("我想自残",
...
def expand_one(user: str, assistant: str, n: int = 8) -> list[tuple[str, str]]:
    """Generate several prompt shapes for one seed.

    We keep the assistant answer canonical most of the time so the persona
    signal stays crisp, but expand the user side into multiple surface forms.
    """
    base = _clean_user(user)
    candidates = [
        base,
        f"请问 {base}",
        f"嗐，{base}",
...
def expand_two_turn_one(user: str, assistant: str) -> list[dict]:
    """Create a small number of two-turn variants for the same intent."""
    base = _clean_user(user)
    prompt_1 = random.choice([
        base,
        f"请问 {base}",
        f"嗐，{base}",
        f"顺手问下：{base}",
    ])
    followup_q = random.choice(FOLLOWUP_USERS)
    if "代码" in base or "git" in base or "Python" in base or "API" in base or "docker" in base:
        followup_a = random.choice([
            "行，先把报错贴出来，再看是环境问题还是逻辑问题。",
            "可以，先复现，再缩小范围，别一上来就乱改。",
            "懂了，先把最小可复现步骤找出来，这比瞎猜快。",
            "好，先别硬扛，按报错往回倒一层层查。",
        ])
    elif any(k in base for k in ["老板", "加班", "会议", "KPI", "屎山", "PUA", "画饼"]):
        followup_a = random.choice([
            "行，记住这句：别硬顶，先保住自己再说。",
            "可以，先把边界说清，别把活全背身上。",
            "懂了，能拖的先拖，能留痕的先留痕。",
        ...
        followup_a = random.choice([
            "行，工友就记一句：先把最急的那步做完，别在原地转圈。",
            "可以，鱼哥给你压成一句：先稳住，再处理最麻烦的那块。",
            "懂了，那就先别硬扛，先把眼前这一关过了。",
            "好，那就按最简单的路子走，别给自己加戏。",
        ])
    return [
...
def main() -> None:
    records: list[dict] = []
    followup_records: list[dict] = []
    for bucket_name, bucket, expansion_n in BUCKETS:
        for user, assistant in bucket:
            user = user.replace("鱼哥", "").replace("工友", "").strip()
            expanded = expand_one(user, assistant, n=expansion_n)
            for u, a in expanded:
                records.append(to_record(u, a))
            # Add a small number of two-turn records so the model sees the
            # persona in a short conversational continuation, not just
            # one-shot instruction/response pairs. Longer buckets get more
            # of these because they carry more style signal.
            followup_rate = 0.45 if bucket_name in {"IDENT", "FORMAT", "MATH"} else 0.8
            if random.random() < followup_rate:
                followup_records.extend(expand_two_turn_one(user, assistant))
            if random.random() < followup_rate / 2:
                followup_records.extend(expand_two_turn_one(user, assistant))
    ...
    bucket_counts = {
        ...
        "CODE": len(CODE), "BOUNDARY": len(BOUNDARY), "MATH": len(MATH),
    ...
    print(f"[moyu] two-turn records : {len(followup_records)}")
